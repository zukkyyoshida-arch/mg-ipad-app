...
export default function PerformanceReport({ ledger, results, prevLedger, prevResults, currentPeriod, onClose }) {
  const analytics = {
    profitRatio: results?.pl?.marginRatio || 0,
    salesGrowth: prevResults ? ((results?.pl?.salesRevenue || 0) - (prevResults?.pl?.salesRevenue || 0)) / (prevResults?.pl?.salesRevenue || 1) * 100 : 0,
    cashFlow: results?.cf?.totalCF || 0
  };
